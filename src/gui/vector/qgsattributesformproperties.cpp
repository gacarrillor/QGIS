...
void QgsAttributesFormProperties::pasteWidgetConfiguration()
{
  ...
  // Get base config from target node and ovewrite settings when possible
  auto config = index.data( QgsAttributesFormModel::NodeFieldConfigRole ).value< QgsAttributesFormTreeData::FieldConfig >();
  ...
  if ( doc.setContent( clipboard->mimeData()->data( QStringLiteral( "application/x-qgsattributetabledesignerelementclipboard" ) ) ) )
  {
    ...
    // When pasting, the target node has already been selected and
